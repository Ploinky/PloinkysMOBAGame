...
namespace PMG {
	...
	public:
		...
		static void PackFile(std::string fileParentDir, std::string fileName, std::vector<uint8_t>* data, std::vector<PakFileTableEntry>* entries) {
			...
			PMG::file_t file = Util::ReadBytesFromFile(fileName);

			int offset = data->size();

			PMG::PakFileTableEntry entry{};
			...
			std::memcpy(data->data() + offset, file.data, file.size);
		}

		static PakFile* UnpackFromPak(std::string pakFileName) {
			PMG::file_t file = Util::ReadBytesFromFile(pakFileName);

			PMG::PakHeader header{};
			int offset = 0;
			std::memcpy(&header.Magic, file.data, 3);
			offset += 3;

			std::memcpy(&header.EntryCount, file.data + offset, sizeof(header.EntryCount));
			offset += sizeof(header.EntryCount);

			std::memcpy(&header.HeaderSize, file.data + offset, sizeof(header.HeaderSize));
			...
			for (int i = 0; i < header.EntryCount; i++) {
				...
				std::memcpy(&entry.FilePathLength, file.data + offset, sizeof(entry.FilePathLength));
				...
				std::memcpy(entry.FilePath.data(), file.data + offset, entry.FilePathLength);
				offset += entry.FilePathLength;

				std::memcpy(&entry.Offset, file.data + offset, sizeof(entry.Offset));
				offset += sizeof(entry.Offset);

				std::memcpy(&entry.Size, file.data + offset, sizeof(entry.Size));
			...
			std::memcpy(pakFile->data.data(), file.data, file.size);
