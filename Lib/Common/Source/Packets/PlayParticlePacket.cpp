...
namespace PMG::Networking {
	void PlayParticlePacket::Read(std::vector<uint8_t>* data) {
		...
		memcpy(particle.data(), data->data() + sizeof(packet_header_t) + sizeof(unit), header.size - sizeof(unit) - sizeof(packet_header_t));
	}
	...
	void PlayParticlePacket::Write(std::vector<uint8_t>* data) {
		...
		memcpy(data->data() + offset + sizeof(packet_header_t) + sizeof(unit), particle.data(), particle.length());
	}
}