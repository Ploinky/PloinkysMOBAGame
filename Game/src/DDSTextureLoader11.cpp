...
namespace
{
        ...
        _Out_opt_ size_t* outNumRows) noexcept
    {
        ...
        if (bc)
        {
            ...
            if (width_ > 0)
            {
                numBlocksWide = std::max<uint64_t>(1u, (uint64_t(width_) + 3u) / 4u);
            }
            uint64_t numBlocksHigh = 0;
            if (height_ > 0)
            {
                numBlocksHigh = std::max<uint64_t>(1u, (uint64_t(height_) + 3u) / 4u);
            }
        ...
        {
            rowBytes = ((uint64_t(width_) + 1u) >> 1) * bpe;
            numRows = uint64_t(height_);
            numBytes = rowBytes * height_;
        }
        else if (fmt == DXGI_FORMAT_NV11)
        {
            rowBytes = ((uint64_t(width_) + 3u) >> 2) * 4u;
            numRows = uint64_t(height_) * 2u; // Direct3D makes this simplifying assumption, although it is larger than the 4:1:1 data
            numBytes = rowBytes * numRows;
        }
        else if (planar)
        {
            rowBytes = ((uint64_t(width_) + 1u) >> 1) * bpe;
            numBytes = (rowBytes * uint64_t(height_)) + ((rowBytes * uint64_t(height_) + 1u) >> 1);
            numRows = height_ + ((uint64_t(height_) + 1u) >> 1);
        }
        else
        {
            ...
            rowBytes = (uint64_t(width_) * bpp + 7u) / 8u; // round up to nearest byte
            numRows = uint64_t(height_);
            numBytes = rowBytes * height_;
        }
...
#undef ISBITMASK
        ...
        _Outptr_opt_ ID3D11ShaderResourceView** textureView) noexcept
    {
        ...
        UINT width_ = header->width_;
        UINT height_ = header->height_;
            ...
            (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC))
        {
            ...
            case D3D11_RESOURCE_DIMENSION_TEXTURE1D:
                ...
                if ((header->flags & DDS_HEIGHT) && height_ != 1)
                {
                    return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
                }
                height_ = depth = 1;
        ...
        case D3D11_RESOURCE_DIMENSION_TEXTURE1D:
            if ((arraySize > D3D11_REQ_TEXTURE1D_ARRAY_AXIS_DIMENSION) ||
                (width_ > D3D11_REQ_TEXTURE1D_U_DIMENSION))
            {
                return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
            }
            break;
        ...
        case D3D11_RESOURCE_DIMENSION_TEXTURE2D:
            if (isCubeMap)
            {
                ...
                if ((arraySize > D3D11_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION) ||
                    (width_ > D3D11_REQ_TEXTURECUBE_DIMENSION) ||
                    (height_ > D3D11_REQ_TEXTURECUBE_DIMENSION))
                {
                    return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
                }
            }
            else if ((arraySize > D3D11_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION) ||
                (width_ > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION) ||
                (height_ > D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION))
            {
                return HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED);
            }
            break;
        ...
        case D3D11_RESOURCE_DIMENSION_TEXTURE3D:
            if ((arraySize > 1) ||
                (width_ > D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION) ||
                (height_ > D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION) ||
        ...
        if (autogen)
        {
            ...
            hr = CreateD3DResources(d3dDevice,
                resDim, width_, height_, depth, 0, arraySize,
                format,
                usage,
            ...
            if (SUCCEEDED(hr))
            {
                size_t numBytes = 0;
                size_t rowBytes = 0;
                hr = GetSurfaceInfo(width_, height_, format, &numBytes, &rowBytes, nullptr);
                if (FAILED(hr))
                    return hr;
        ...
        else
        {
            ...
            size_t theight = 0;
            size_t tdepth = 0;
            hr = FillInitData(width_, height_, depth, mipCount, arraySize,
            ...
            if (SUCCEEDED(hr))
            {
                ...
                if (FAILED(hr) && !maxsize && (mipCount > 1))
                {
                    ...
                    hr = FillInitData(width_, height_, depth, mipCount, arraySize, format, maxsize, bitSize, bitData,
                        twidth, theight, tdepth, skipMip, initData.get());
                    if (SUCCEEDED(hr))
                    {
                        hr = CreateD3DResources(d3dDevice,
