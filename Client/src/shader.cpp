...
namespace PMG {
	...
	ColorShader::~ColorShader() {
		...
		m_frameConstBuffer = 0;

		m_modelConstBuffer->Release();
		m_modelConstBuffer = 0;
	}
	...
	void TextureShader::Initialize(Direct3D* direct3D) {
		...
		Shader::Initialize(direct3D, "./shaders/texture_vs.cso", "./shaders/texture_ps.cso", inputLayoutDesc, sizeof(inputLayoutDesc) / sizeof(D3D11_INPUT_ELEMENT_DESC));
	...
	SkinnedTexturedShader::SkinnedTexturedShader() : Shader(ShaderType::SKINNED_TEXTURED) {

	}

	SkinnedTexturedShader::~SkinnedTexturedShader() {
		m_frameConstBuffer->Release();
		m_frameConstBuffer = 0;
	...
	void SkinnedTexturedShader::Initialize(Direct3D* direct3D) {
		D3D11_INPUT_ELEMENT_DESC inputLayoutDesc[]{
			{"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0},
			{"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0},
			{"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0},
			{"BONE_INDICES", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0},
			{"BONE_WEIGHTS", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA, 0},
		};

		Shader::Initialize(direct3D, "./shaders/skinned_textured_vs.cso", "./shaders/texture_ps.cso", inputLayoutDesc, 5);
