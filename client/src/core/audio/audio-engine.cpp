...
bool CAudioEngine::Initialize(CClientAssetManager* pAssetManager) {
...
void CAudioEngine::SetMasterVolume(float fValue) {
    fValue = std::min(1.0f, std::max(fValue, 0.0f));

    pMasterVoice->SetVolume(fValue);
}

HVoice CAudioEngine::CreateVoice(SoundAsset_t sound) {
...
HEmitter CAudioEngine::CreateEmitter(Vector3 vec3InitialPos) {
...
void CAudioEngine::UpdateVoicePosition(HVoice hVoice, HEmitter hEmitter, Vector3 vec3Position) {
