...
namespace PMG {
    ...
    void ServerNetworkManager::OnConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t* callback) {
        if (callback->m_info.m_hListenSocket && callback->m_eOldState == k_ESteamNetworkingConnectionState_None
            ...
            peer->connection = callback->m_hConn;
            ...
            for (NetworkPeer* peer : clients_) {
                if (peer->connection == callback->m_hConn) {
                    on_clientDisconnected(peer->steamId);
    ...
    bool ServerNetworkManager::Close() {
        ...
        for (NetworkPeer* peer : clients_) {
            SteamNetworkingSockets()->CloseConnection(peer->connection, 0, nullptr, false);
            delete peer;
        }
    ...
    void ServerNetworkManager::Update() {
        for (NetworkPeer* peer : clients_) {
            ...
            while (ReceivePacket(peer->steamId, &packet)) {
                on_clientMessageReceived(peer->steamId, &packet);
            }
        }
    }
    ...
    void ServerNetworkManager::SendToAllClients(std::vector<uint8_t>* data) {
        for (NetworkPeer* peer : clients_) {
            SendToClient(peer->steamId, data);
        }
    }

    void ServerNetworkManager::SendToAllClients(Networking::BasePacket* packet) {
        for (NetworkPeer* peer : clients_) {
            SendToClient(peer->steamId, packet);
        }
    }

    void ServerNetworkManager::SendToClient(PlayerID playerId, Networking::BasePacket* packet) {
