...
namespace PMG {
    ...
    void Client::Run(std::string connectString_) {
        ...
        if (connectString_.length() > 0) {
            ServerNetworkManager* serverNetworkManager = new ServerNetworkManager();
            serverNetworkManager->ConnectToServer(connectString_);
                currentState_ = new Game(serverNetworkManager, this, window->width_, window->height_);
        }
        else {
            currentState_ = new CLoadingState(this, window->width_, window->height_);
        }

        std::vector<PMGSystem*> systems;

        std::string strFileName = TEXT("audio.wav");
        AudioComponent comp;
    ...
    void Client::RequestCursor(CursorId newCursor) {
        ...
        case CursorId::BUTTON_HOVER: {
            SetCursor(LoadCursor(GetModuleHandleA(NULL), MAKEINTRESOURCE(IDC_CURSOR2)));
            break;
        }
        case CursorId::ATTACK_MOVE: {
            SetCursor(LoadCursor(GetModuleHandleA(NULL), MAKEINTRESOURCE(IDC_CURSOR3)));
            break;
        }
        case CursorId::DEFAULT:
        default: {
            SetCursor(LoadCursor(GetModuleHandleA(NULL), MAKEINTRESOURCE(IDC_CURSOR1)));
        }
        }
    }
    ...
    void Client::JoinGame(ServerNetworkManager* networkManager) {
