...
namespace PMG {
	Lobby::Lobby(std::string server, IClientStateHandler* handler, int width, int height) : IClientState(handler, width, height) {
        ...
        packetManager_.RegisterHandler(Networking::PacketType::LOBBY_PLAYER_PCK, [this](std::vector<uint8_t> data) { HandleSlotPacket(data); });
        packetManager_.RegisterHandler(Networking::PacketType::LOBBY_GAME_START, [this](std::vector<uint8_t> data) { HandleGameStartPacket(data); });
    ...
    void Lobby::HandleGameStartPacket(std::vector<uint8_t> data) {
        Networking::LobbyGameStartPck pck = Networking::LobbyGameStartPck();
        pck.Read(&data);
