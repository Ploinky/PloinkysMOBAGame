...
namespace PMG {
    ...
    bool Direct3D::CreateSwapChain(bool full_screen) {
        ...
        hr = dxgiFactory->CreateSwapChainForHwnd(device, windowHandle, &sc, &fsc, nullptr, &swapChain);
	...
	bool Direct3D::CreateRasterizerState() {
		...
		HRESULT hr = device->CreateRasterizerState(&rasterizerDesc, &rasterizerState_);
    ...
    void Direct3D::EnableAlphaBlending() {
        float input[4] = { 0,0,0,0 };
        context->OMSetBlendState(alpha_blend_state, 0, 0xffffffff);
    }
    ...
    void Direct3D::SetWindowDimensions(int width_, int height_) {
